...
3M|M*Modal
Pittsburgh,PA
#changes amfm_decompy/basic_tools.py to suit wavfiles and output as MATLAB version
#imports readwav_structobject.py instead of amfm_decompy/basic_tools.py
"""
#Modified from original whose details is as below
"""
...
#    parameters['f0_max'] = kwargs.get('f0_max', 400.0)              #Maximum F0 searched (Hz)
    parameters['tda_frame_length'] = \
                              kwargs.get('tda_frame_length', 35.0)  #Frame length employed in the time domain analysis (ms)
    parameters['frame_space'] = kwargs.get('frame_space', 10.0)     #Spacing between analysis frames (ms)
    parameters['f0_min'] = kwargs.get('f0_min', 50.0)               #Minimum F0 searched (Hz)
    parameters['f0_max'] = kwargs.get('f0_max', 500.0)              #Maximum F0 searched (Hz)
    ...
    nonlinear_sign = wavstruct.SignalObj(signal.data**2, signal.fs)
...
def nlfer(signal, pitch, parameters):
    ...
    # Set parameters.
    #---------------------------------------------------------------
    N_f0_min = np.around((parameters['f0_min']*2/float(signal.fs))*pitch.nfft)
    N_f0_max = np.around((parameters['f0_max']/float(signal.fs))*pitch.nfft)

    window = hanning(pitch.frame_size+2)[1:-1]
    data = np.zeros((signal.size))  #Needs other array, otherwise stride and
    data[:] = signal.data     #windowing will modify signal.filtered
...
def time_track(signal, spec_pitch, pitch_std, pitch, parameters):
    ...
    for frame in range(tda_nframes):
        lag_min0 = (int(np.fix(signal.fs/spec_range[1, frame])) -
                                    int(np.fix(parameters['nccf_pwidth']/2.0)))
        lag_max0 = (int(np.fix(signal.fs/spec_range[0, frame])) +
                                    int(np.fix(parameters['nccf_pwidth']/2.0)))

        phi = crs_corr(signal_frames[frame, :], lag_min0, lag_max0)
        time_pitch[:, frame], time_merit[:, frame] = \
            cmp_rate(phi, signal.fs, maxcands, lag_min0, lag_max0, parameters)
